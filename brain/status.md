...
| Item | Situação |
|---|---|
| Domínio | ⏳ **`cavlrv.com.br` registrado; delegação para a Vercel salva na tela certa do Registro.br** (print do Rafael confirma ns1/ns2.vercel-dns.com + aviso de transição de ~2h). Diagnóstico de 2026-09-02, pós-janela: registro `.br` ainda publica os NS antigos (whois `changed: 20260902`, sem DS/DNSSEC travando), enquanto os NS da Vercel já respondem com autoridade (flag `aa` + SOA — exatamente o teste que o Registro.br faz). Conclusão: lado nosso 100% pronto; falta a publicação de zona do registro, que atrasa além do contador. Monitor da sessão segue vigiando; ao virar: verify + TLS + redeploy |
