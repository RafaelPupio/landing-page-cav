...
| Item | Situação |
|---|---|
| Domínio | ⏳ **`cavlrv.com.br` REGISTRADO** (RDAP oficial confirma, 2026-09-02) e apontado ao projeto na Vercel. Falta só o DNS chegar: Rafael diz ter alterado, mas os nameservers públicos ainda são `a/b.auto.dns.br` (padrão do Registro.br) com zona vazia — ou está propagando (~2h) ou foi salvo na tela errada (a certa é "Alterar servidores DNS" → `ns1/ns2.vercel-dns.com`; "Modo avançado" de registros é outra coisa). Monitor ativo na sessão de 2026-09-02 vigiando a virada; quando virar: `vercel domains verify`, TLS/cabeçalhos e **redeploy** (metadados resolvem no build) |
