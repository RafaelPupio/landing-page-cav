...
| Item | Situação |
|---|---|
| Domínio | ⏳ **`cavlrv.com.br` escolhido; registro em andamento pela contabilidade da igreja, no CNPJ dela** (decidido 2026-09-02 — Rafael ainda NÃO registrou; a menção anterior a "registrado" era prematura, e é por isso que o RDAP não mostrava nada). Lado Vercel já pronto: apex + www adicionados ao `landing-page-cav`, aguardando. Quando o registro confirmar, falta um passo manual no painel do Registro.br: trocar os servidores DNS para `ns1.vercel-dns.com` / `ns2.vercel-dns.com`. Depois: `vercel domains verify`, conferir TLS/cabeçalhos e **redeploy** (canonical/og/sitemap resolvem no build — lição de 2026-08-11) |
