...
| Item | Situação |
|---|---|
| Domínio | ❌ **A igreja segue sem domínio.** `cav.com.br` pertence a um revendedor de domínios — redireciona para página de venda em brand.com.br (constatado 2026-09-02). Domínio de 3 letras nessas condições custa milhares de reais, não os R$40 normais; descartado. Livres no Registro.br em 2026-09-02 (via RDAP oficial): `arvoredavidalrv.com.br`, `cavlrv.com.br`, `igrejacav.com.br`, `comunidadecav.com.br` (~R$40/ano cada; a Vercel não registra `.com.br`). O `rafaelpupio.com` da conta é do site pessoal do Rafael — **pago por ele no cartão** (não grátis, como registrado antes por engano) — e já está aliasado ao projeto `rafaelpupio-com`. `lib/urlDoSite.ts` adota sozinho o que for apontado |
