...
**O link do YouTube estava quebrado em produção.** Rafael pediu para trocar pelo canal `@ÁrvoredaVida-u9t`; ao conferir, o endereço que estava no ar (`@ArvoredaVidaLRV`, extraído da bio do Instagram em 2026-08-08) respondia **404**. Ou seja, não era preferência: era conserto de link morto, em três lugares do `site.json` mais o `sameAs` do JSON-LD.

O handle novo tem acento (`Á`). Guardado cru no JSON, que é o que a pessoa da igreja lê no `/editar`; o navegador percent-encoda sozinho no `href` (`%C3%81`), e o `z.string().url()` aceita. Conferido no ar: 200.

**Menu do celular não recolhia.** Era um `<details>` puro dentro do `Cabecalho` (componente de servidor): abria e fechava no botão, mas continuava aberto depois de escolher um item — cobrindo justamente a seção que a pessoa acabou de pedir.

Extraído para `components/ui/MenuCelular.tsx`, client component. **Continua sendo um `<details>` nativo** — antes da hidratação abre e fecha sem JavaScript. O que o componente acrescenta é o fechamento: ao escolher item, com Esc (devolvendo o foco ao botão) e ao tocar fora. O ícone também vira um "X" quando aberto.

Verificado no navegador, não só em teste: abre, navega para a âncora e recolhe.


## 2026-09-02 — Spotify no capítulo de Mensagens, com uma correção de expectativa

Rafael pediu "um espaço para o Spotify para acompanharem as palavras mais recentes" e mandou o perfil da igreja.

**Duas checagens antes de construir, e as duas mudaram o resultado.**

1. **Perfil de usuário do Spotify não é incorporável.** `open.spotify.com/embed/user/<id>` devolve 404 — o embed só aceita playlist, álbum, faixa, artista, podcast e episódio. Os 200 que `embed/playlist` e `embed/show` devolvem para um id inválido são a casca do aplicativo, não conteúdo. Por isso o padrão virou botão, com `spotifyEmbedUrl` reservado para quando a igreja escolher UMA playlist para destacar.

2. **O Spotify da igreja não tem pregação.** Abri o perfil no navegador: 9 playlists públicas, todas de música — `kids day`, `culto de mulheres`, `A Benção (Infantil)`, `Fundo Palavra`, `Secreto com Deus`, `Natal`, `INTIMIDADE`, `OTB Connect`, `Árvore da Vida`. "Fundo Palavra" é o som que toca **durante** a mensagem, não a mensagem. As pregações estão no YouTube.

Então o texto no site descreve o que existe de verdade ("as playlists de louvor da comunidade"), em vez de prometer pregações que o visitante não encontraria. Levantado com o Rafael para ele corrigir se houver um podcast que não achei.

**O `si=` do link foi removido.** O endereço compartilhado vinha com `?si=a8582543571d4d79`, que é token de rastreio de quem compartilhou — publicá-lo faria cada clique de visitante carregar o identificador do Rafael. Travado por teste sobre o conteúdo real.

Spotify entrou também no rodapé e no `sameAs` do JSON-LD, ambos com degradação: rede vazia some, em vez de virar link morto ou `sameAs` com string vazia (que confunde o Google em vez de ajudar).


## 2026-09-02 — Atalhos do topo em duas faixas, redes com cor de plataforma

Rafael pediu "3 em cima e 3 embaixo" mas listou **quatro** itens em cima. Em vez de escolher no escuro, montei as duas leituras numa maquete (`docs/mockups/acoes/`) e mostrei antes de executar. Ele escolheu **A** (4+3) com a escrita compacta da **B**.

**A medição que decidiu o formato:** três colunas numa tela de 375px dão 103px por cartão, e "No que cremos", "Como chegar" e "Cultos aos domingos" quebram em duas linhas. Em duas colunas dão 159px e cabem numa linha. Por isso a faixa principal é `grid-cols-2 md:grid-cols-4`, não três colunas — o "3 em cima" do pedido vira 2×2 no celular e 4 em linha no desktop.

Escrita compacta da B aplicada aos quatro: `Cultos / Dom, 18h` e `Santa Ceia / 1º dom, 8h`, com o horário num subtítulo menor em vez de dentro do rótulo.

`acoesRapidas` ganhou `detalhe` (subtítulo, vazio some) e `grupo` (`principal` ou `rede`, que divide as duas faixas). O enum de ícone ganhou `credo`, `ceia` e `spotify`.

"No que cremos" aponta para `#credo` — confirmado com o Rafael antes de implementar. É o único atalho da faixa principal que não vai para `#visita`.

Cores das plataformas e a correção do roxo do Instagram: ver [[projeto/identidade-visual]].

**Detalhe de implementação que vale lembrar:** o `<linearGradient>` do Instagram tem id fixo. Dois gradientes com o mesmo id no documento fariam o navegador usar sempre o primeiro — há teste verificando que o id não se repete.

**Ajuste de tamanho, no mesmo dia.** O primeiro corte deixou os cartões com ~100px de altura e o conteúdo alinhado à esquerda; o Rafael pediu menores, mais bem encaixados e centralizados. Passaram a `items-center justify-center`, ícone de 24px para 20px, rótulo `text-[13px]`, subtítulo `text-[11px]`, `rounded-xl` no lugar de `rounded-2xl` (canto grande num cartão baixo faz o retângulo parecer oval). Medido no navegador a 375px: **68px de altura** (84px nos dois que têm horário), faixa inteira em **277px** — cabe na primeira dobra junto com o cabeçalho.


## 2026-09-03 — Ícone do Spotify: traçado oficial, e o ajuste ótico que ele exigiu

Rafael: "o logo do Spotify parece estranho". Estava — e a causa era minha: o traçado era **desenho à mão**, não a marca. As três ondas saíram finas demais e mal curvadas, então a 20px o ícone lia como um disco verde com riscos, não como o Spotify. Instagram e YouTube não tinham esse problema porque a geometria deles é simples o bastante para reproduzir de cabeça; a do Spotify não é.

Trocado pelo traçado oficial da marca (simple-icons). Verificado no navegador ampliado a 110px antes de aceitar — não só a 20px, onde um traçado errado ainda pode parecer aceitável.

**A troca criou um segundo problema, que só a medição pegou.** O traçado oficial preenche a caixa inteira: 24×24 de tinta, contra 20×20 do Instagram e 20×14 do YouTube dentro da mesma `viewBox`. Resultado: o Spotify passou a ser 20% maior que os vizinhos e dominava a faixa.

Daí `AJUSTE_OTICO`, aplicado só ao Spotify: `translate(2 2) scale(0.8333)`. Depois dele os três ícones medem 16,7px de largura renderizada — medido no navegador com `getBoundingClientRect` no `<path>`, não estimado. Travado por teste: o Spotify tem o transform, Instagram e YouTube não têm nenhum.

**A lição que vale além deste ícone:** logo de plataforma se copia da fonte, não se desenha de memória. E ao trocar um traçado por outro, comparar a caixa de tinta — duas `viewBox` iguais não significam dois ícones do mesmo tamanho.


## 2026-09-03 — Rótulos dos botões de Mensagens, e a contradição que eles criaram

Rafael pediu dois rótulos mais explícitos em `mensagens`:

- `canalTexto`: "Ver as mensagens no YouTube" → **"Assista às nossas mensagens no YouTube"**
- `spotifyTexto`: "Ouvir no Spotify" → **"Ouça as nossas mensagens no Spotify"**

Só `content/site.json` mudou — os testes usam fixtures próprias, então nada quebrou. A 375px os dois botões passam a quebrar em duas linhas; a pílula acomoda bem, e eles já ficam empilhados no celular.

**O que ficou pendente com o Rafael:** o novo rótulo do Spotify promete *mensagens*, mas a linha logo abaixo (`spotifyIntro`) diz "As playlists de louvor da comunidade — as músicas dos nossos cultos", e a verificação de 2026-09-02 achou 9 playlists no perfil, todas de música, nenhuma pregação. Botão e legenda agora se contradizem na tela.

Não mexi na legenda por conta própria porque há duas saídas válidas e a escolha é dele: ou o Spotify passou a ter pregação (e a legenda é que está velha), ou continua só louvor (e o rótulo promete o que o visitante não encontra). Levantado; a mudança pedida foi entregue como pedida.
